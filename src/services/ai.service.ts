...
// ======================== INVENTORY FORECAST ========================
export interface SimilarSnapshot {
  snapshotTime: string;
...
export interface InventoryForecastResponse {
  ...
  similarSnapshots?: SimilarSnapshot[];
}

export async function aiInventoryForecast(items: Array<{ code: string; name: string; quantity: number; avgDailySales?: number }>): Promise<InventoryForecastResponse> {
  const response = await apiFetch<ApiResponse<InventoryForecastResponse>>('/api/ai/inventory-forecast', {
    method: 'POST',
...
export interface SimilarReceipt {
  id?: number | null;
  score?: number | null;
  receiptType?: 'IMPORT' | 'EXPORT' | string | null;
...
export interface ReceiptOCRResponse {
  ...
  similarReceipts?: SimilarReceipt[] | null;
}
