...
// Page type cho pagination (giống Spring Data Page)
export type Page<T> = {
    content: T[];
...
/* ============================================================
   GỘP TẤT CẢ PHIẾU NHẬP (NCC, NỘI BỘ, NVBH) - VỚI PAGINATION
============================================================ */
export async function getAllImports(params?: {
    ...
    if (typeof params?.supplierId === 'number') {
        qs.set("supplierId", String(params.supplierId));
    }
    if (typeof params?.storeId === 'number') {
        qs.set("storeId", String(params.storeId));
    }
    ...
    const url = `${API_BASE}/api/imports?${qs.toString()}`;
    ...
    if (!res.ok) {
        ...
        console.error('❌ getAllImports error:', res.status, errorText);
        throw new Error(`Không lấy được danh sách phiếu nhập (${res.status})`);
    }

    const json: ApiResponse<Page<SupplierImport>> = await res.json();
    console.log('📦 getAllImports response:', json);
    console.log('📦 getAllImports json.data:', json.data);
    console.log('📦 getAllImports json.data type:', typeof json.data, Array.isArray(json.data));
    
    if (!json.data) {
        console.error('❌ getAllImports: json.data is null/undefined', json);
        throw new Error('Response không có data');
    }
    
    // Nếu json.data là Array, có thể backend trả về trực tiếp array thay vì Page object
    if (Array.isArray(json.data)) {
        console.warn('⚠️ getAllImports: json.data is Array, converting to Page format');
        return {
            content: json.data,
...
/* ============================================================
   GỘP TẤT CẢ PHIẾU XUẤT (NCC, NỘI BỘ, NVBH) - VỚI PAGINATION
============================================================ */
export async function getAllExports(params?: {
    ...
    const url = `${API_BASE}/api/exports?${qs.toString()}`;
    ...
    if (!res.ok) {
        const errorText = await res.text();
        console.error('❌ getAllExports error:', res.status, errorText);
        throw new Error(`Không lấy được danh sách phiếu xuất (${res.status})`);
    }

    const json: ApiResponse<Page<SupplierExport>> = await res.json();
    console.log('📤 getAllExports response:', json);
    console.log('📤 getAllExports json.data:', json.data);
    console.log('📤 getAllExports json.data type:', typeof json.data, Array.isArray(json.data));
    
    if (!json.data) {
        console.error('❌ getAllExports: json.data is null/undefined', json);
        throw new Error('Response không có data');
    }
    
    // Nếu json.data là Array, có thể backend trả về trực tiếp array thay vì Page object
    // Hoặc Page object bị serialize thành array
    if (Array.isArray(json.data)) {
        console.warn('⚠️ getAllExports: json.data is Array, converting to Page format');
        return {
            content: json.data,
            totalElements: json.data.length,
            totalPages: 1,
...
export async function getImportById(id: number): Promise<SupplierImport> {
    ...
    if (!res.ok) {
        throw new Error("Không lấy được chi tiết phiếu nhập");
    }
...
export async function getInventoryChecks(params?: {
    ...
    const url = `${API_BASE}/api/inventory-checks?${qs.toString()}`;
    ...
    if (!res.ok) {
        const errorText = await res.text();
        console.error('❌ getInventoryChecks error:', res.status, errorText);
        throw new Error(`Không lấy được danh sách phiếu kiểm kê (${res.status})`);
    }

    // Backend trả về { success: true, data: Page<InventoryCheckDto> }
    const json: { success: boolean; data: Page<InventoryCheck> } = await res.json();
    console.log('✅ getInventoryChecks response:', json);
    console.log('✅ getInventoryChecks json.data:', json.data);
    console.log('✅ getInventoryChecks json.data type:', typeof json.data, Array.isArray(json.data));
    
    if (!json.data) {
        console.error('❌ getInventoryChecks: json.data is null/undefined', json);
        throw new Error('Response không có data');
    }
    
    // Nếu json.data là Array, có thể backend trả về trực tiếp array thay vì Page object
    if (Array.isArray(json.data)) {
        console.warn('⚠️ getInventoryChecks: json.data is Array, converting to Page format');
        return {
            content: json.data,
            totalElements: json.data.length,
            totalPages: 1,
...
export async function getDashboardStats(): Promise<DashboardStats> {
    ...
    if (!res.ok) {
        let msg = 'Không lấy được thống kê dashboard';
        try {
            const errorText = await res.text();
            ...
            try {
                ...
                msg = `HTTP ${res.status}: ${errorText.substring(0, 100)}`;
            }
        } catch {
