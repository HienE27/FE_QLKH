...
export type ProductSearchParams = {
  ...
  page?: number;
  size?: number;
  sort?: string;     // Format: "field,direction" (e.g., "name,asc" or "unitPrice,desc")
};
...
function buildQuery(params: ProductSearchParams): string {
  ...
  // Luôn gửi page và size (backend Spring Data yêu cầu)
...
export async function searchProducts(
  ...
  // Backend endpoint là /api/products/search
  // Backend trả về Page<ProductDto> trực tiếp, nhưng API Gateway có thể wrap trong ApiResponse
  const res = await apiFetch<ApiResponse<ProductPage> | ProductPage>(`${BASE_URL}/search${query}`);
  
  // Kiểm tra xem có wrap trong ApiResponse không
  if ('data' in res && res.data) {
  return res.data;
  }
  // Nếu không wrap, trả về trực tiếp
