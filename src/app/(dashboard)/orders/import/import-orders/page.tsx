...
import { getImportOrders, type ImportOrder, type ExportStatus } from '@/services/inventory.service';
...
export default function ImportOrdersPage() {
    ...
    const loadData = async () => {
        try {
            setLoading(true);
            const result = await getImportOrders();

            // Fetch thông tin supplier cho các lệnh nhập nếu thiếu
            const { getSuppliers } = await import('@/services/supplier.service');
            const suppliers = await getSuppliers();

            // Map lại dữ liệu với thông tin supplier
            const mappedData = result.map(order => {
            ...
            console.error('Lỗi khi tải danh sách lệnh nhập:', error);
            setData([]);
            // Không hiển thị alert để tránh làm phiền user khi API chưa sẵn sàng
            // alert('Không thể tải danh sách lệnh nhập kho');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSearch = async () => {
        try {
            ...
            setData([]);
            alert('Không thể tìm kiếm lệnh nhập');
        } finally {
            setLoading(false);
        }
    };
                                ...
                                {loading ? (
                                    ...
                                    currentData.map((record, index) => (
                                        <tr key={`${record.id}-${index}`} className="border-b border-gray-200 hover:bg-gray-50 transition-colors h-[48px]">
                                            ...
                                            <td className="px-4 text-center text-sm">{record.supplierName || 'N/A'}</td>
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (
                                                                ...
                                                                router.push(`/orders/import/edit-import-order/${record.id}`);
                                                            }
                                                        }}
                                                        disabled={
                                                            record.status === 'APPROVED' ||
