...
import { getStaffImports, type StaffImport, type ExportStatus } from '@/services/inventory.service';
...
export default function PhieuNhapNhanVien() {
    const router = useRouter();
    const [data, setData] = useState<StaffImport[]>([]);
    ...
    const loadImports = async () => {
        try {
            setLoading(true);
            setError(null);
            const imports: StaffImport[] = await getStaffImports({
            ...
            const filtered = staffFilter === 'ALL'
                ? imports
                : imports.filter(im => im.staffId === staffFilter);
            ...
            if (err instanceof Error) {
                ...
                setError('Lỗi tải danh sách phiếu nhập nhân viên');
            }
            console.error(err);
        } finally {
            setLoading(false);
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (
                                                                ...
                                                                router.push(
                                                                    `/dashboard/products/import-nvbh/edit-import-nvbh-receipt/${record.id}`,
                                                                );
                                                            }
                                                        }}
                                                        disabled={
