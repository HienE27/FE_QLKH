...
import { getInventoryChecks, deleteInventoryCheck, type InventoryCheck, type InventoryCheckStatus, type Page } from '@/services/inventory.service';
...
export default function InventoryChecksPage() {
    ...
    const [checkPage, setCheckPage] = useState<Page<InventoryCheck> | null>(null);
    ...
    const loadChecks = async () => {
        try {
            ...
            const pageResult = await getInventoryChecks({
                status: filterStatus === 'ALL' ? 'ALL' : filterStatus,
                checkCode: filterCode || undefined,
                fromDate: filterFromDate || undefined,
                toDate: filterToDate || undefined,
                storeId: undefined, // TODO: Thêm store filter nếu cần
                page: currentPage - 1, // Backend dùng 0-based
                size: itemsPerPage,
                sort: sortStr,
            });

            console.log('✅ Loaded checks page:', pageResult);
            setCheckPage(pageResult);
        } catch (err) {
    ...
    // Reload từ BE khi pagination hoặc sort thay đổi
    useEffect(() => {
        loadChecks();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filterStatus, filterCode, filterFromDate, filterToDate]);
    ...
    // Pagination từ BE
    // Nếu checkPage là Array (fallback), dùng trực tiếp
    const currentData = Array.isArray(checkPage)
        ? checkPage
        : (checkPage?.content || []);
    const totalItems = Array.isArray(checkPage)
        ? checkPage.length
        : (checkPage?.totalElements || 0);
    const totalPages = Array.isArray(checkPage)
        ? 1
        : (checkPage?.totalPages || 1);
